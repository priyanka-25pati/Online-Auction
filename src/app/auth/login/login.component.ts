...
export class LoginComponent implements OnInit {
 ...
 constructor(
    ...
    private authService: AuthService, // Assuming this is the correct service for login logic
 ...
 onSubmit() {
  ...
  this.authService.loginUser(credentials).subscribe({
     next: (response: AuthResponse) => {
       ...
       if (response.user) {
         // Correctly access userID from the user object
         ...
         // Fallback: If neither a user nor an admin is found, show an alert and redirect to RegisterComponent
         alert('Login successful, but no user or admin information found. Redirecting to registration.');
     ...
     error: (error) => {
       ...
       // Directly navigate to the sign-up page if login fails
       this.router.navigate(['/register']);
  }
  });
 }
